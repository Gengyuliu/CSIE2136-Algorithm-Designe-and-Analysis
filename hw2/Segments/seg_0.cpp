...
namespace Memory {
    constexpr int N = 200005; // Number of cells, change this if you need more cells
...
/*	check if we could fill certain V line segments over [0,M]	*/
/*	0 : fail  1 : success	*/
int check(Segment *S, ll *chose_seg, ll V,  int N, int M, ll K){
    /*	Two pointers	*/
...
void max_range_cover(Segment *S, int N, int M, ll K){
    ...
    //Not handle V = K
    for (l = 0, r = K; l+1 < r;){   /*	logK	*/
        V = (l + r)>>1;
        /*	if m is valid --> l = m+1	*/
        if (check(S, chose_seg, V, N, M, K))
            l = V;      /*	l : valid	*/
        else
            r = V;      /*	r : invalid	*/
    }
    /*	the ans is l	*/ 
    check(S, chose_seg, l, N, M, K);
    printf("%lld\n", l);
    for (i = 0; i < N; ++i){
        printf("%lld ", chose_seg[i]);
    } 
